...
def draw_line_bres2(windows, color, p_ini, p_fin):
    x1=p_ini[0]
    y1=p_ini[1]
    x2=p_fin[0]
    y2=p_fin[1]
...
def Rectangulo_Borde(windows, color, pos_ini, pos_fin):
    ...
    X1= pos_fin[0]
    Y1= pos_fin[1]
    draw_line_bres2(windows, color, [X0, Y0],[X1,Y0])
    draw_line_bres2(windows, color, [X0, Y1],[X1,Y1])
    draw_line_bres2(windows, color, [X0, Y0],[X0,Y1])
    draw_line_bres2(windows, color, [X1, Y0],[X1,Y1])
...
def Rectangulo_Relleno(windows,colorb,colorr,pos_ini,pos_fin):
    ...
    X1= pos_fin[0]
    Y1= pos_fin[1]
    draw_line_bres(windows, colorr, [X0, Y0],[X1,Y0])
    draw_line_bres(windows, colorr, [X0, Y1],[X1,Y1])
    draw_line_bres(windows, colorr, [X0, Y0],[X0,Y1])
    draw_line_bres(windows, colorr, [X1, Y0],[X1,Y1])
    Y0= Y0 +1
    while Y0 <Y1:
        draw_line_bres(windows, colorb, [X0+1, Y0],[X1-1,Y0])
        Y0=Y0+1
...
def pentagono(windows,colorb,x_ini,y_ini,base,altu):
    ...
    Y0= y_ini
    draw_line_bres(windows, colorb, [X0, Y0],[X0+base,Y0])
    draw_line_bres(windows, colorb, [X0, Y0],[X0-(X0*0.1),altu])
    draw_line_bres(windows, colorb, [X0+base, Y0],[base+X0+(X0*0.1),altu])

    draw_line_bres(windows, colorb, [X0-(X0*0.1),altu],[X0+(base/2),altu-10])
    draw_line_bres(windows, colorb, [base+X0+(X0*0.1),altu],[X0+(base/2),altu-10])
